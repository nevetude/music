...
function create_context(get_context, set_context, has_context) {
  ...
  return [
    ...
    (context) => set_context(key2, context),
    () => has_context(key2)
  ];
}
...
process_fn = function() {
  ...
  while (__privateGet(this, _scheduled).length > 0) {
    ...
    for (const root of __privateMethod(this, _Batch_instances, resolve_fn).call(this)) {
      try {
  ...
  for (let batch = first_batch; batch !== null; batch = __privateGet(batch, _next)) {
    ...
    if (others.length === 0) {
      ...
      if (true_default && !__privateGet(batch, _decrement_queued)) {
        invariant(__privateGet(batch, _scheduled).length === 0, "Batch has scheduled effects");
      }
      if (is_earlier) {
      ...
      if (__privateGet(batch, _scheduled).length > 0 && !__privateGet(batch, _decrement_queued)) {
        batch.apply();
        for (var root of __privateMethod(_a3 = batch, _Batch_instances, resolve_fn).call(_a3)) {
          __privateMethod(_b = batch, _Batch_instances, traverse_fn).call(_b, root, [], []);
        }
      }
      batch.deactivate();
    }
  ...
  return new Proxy(
    ...
    value,
    {
      ...
      getOwnPropertyDescriptor(target, prop2) {
        var _a3;
        (_a3 = this.has) == null ? void 0 : _a3.call(this, target, prop2);
